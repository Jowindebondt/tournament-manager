...
public class UT_PouleRepository
{
    ...
    private readonly PouleRepository _repo;
    ...
    [Fact]
    public async Task GetAllByRoundAndTournamentAsync_NullTournamentId_ThrowsArgumentNullException()
    {
        // arrange
        var roundId = new RoundId(Guid.NewGuid());

        // act & assert
        await Assert.ThrowsAsync<ArgumentNullException>(async () => await _repo.GetAllByRoundAndTournamentAsync(null!, roundId));
    }

    [Fact]
    public async Task GetAllByRoundAndTournamentAsync_NullRoundId_ThrowsArgumentNullException()
    {
        // arrange
        var tournamentId = new TournamentId(Guid.NewGuid());

        // act & assert
        await Assert.ThrowsAsync<ArgumentNullException>(async () => await _repo.GetAllByRoundAndTournamentAsync(tournamentId, null!));
    }
