...
public class UT_RoundName
{
    [Fact]
    public void Create_ValidValue_ReturnsRoundName()
    {
        // arrange
        var str = "abc";

        // act
        var value = RoundName.Create(str);
    ...
    public void Create_InvalidValue_ThrowsArgumentException(int stringLength)
    {
        ...
        // act & assert
        Assert.Throws<ArgumentException>(() => RoundName.Create(str));
    }
    ...
    {
        // arrange
        var tnA = RoundName.Create(a);
        var tnB = RoundName.Create(b);
